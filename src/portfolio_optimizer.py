...
class PortfolioOptimizer:
    ...
    def _solve_with_alternative_method(self, annual_mean: pd.Series,
        ...
        portfolio_return = annual_mean.values @ equal_weights
        portfolio_vol = np.sqrt(equal_weights.T @ cov_matrix.values @ equal_weights)
        ...
        try:
            ...
            portfolio_return_uncon = annual_mean.values @ w_unconstrained
            portfolio_vol_uncon = np.sqrt(w_unconstrained.T @ cov_matrix.values @ w_unconstrained)
        ...
        for target in target_returns:
            try:
                ...
                if problem.status in [cp.OPTIMAL, cp.OPTIMAL_INACCURATE]:
                    risks.append(np.sqrt(portfolio_vol.value))
    ...
    def get_optimization_summary(self, weights: np.ndarray, 
        ...
        """
        # 计算组合收益和风险
        portfolio_return = annual_mean.values @ weights
...
def get_portfolio_optimizer(risk_free_rate: float = 0.02) -> PortfolioOptimizer:
    """获取投资组合优化器实例"""
    return PortfolioOptimizer(risk_free_rate)